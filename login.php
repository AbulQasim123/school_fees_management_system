...
if (isset($_POST['login_button'])) {
	...
	if ($message == '') {
		...
		$query = "SELECT * FROM sfms_admin WHERE admin_email = :admin_email";
			...
			foreach ($statement->fetchAll() as $row) {
				if ($row['admin_status'] == 'Enable') {
					if ($row['admin_password'] == $formdata['admin_password']) {
						...
						$message = '<li>Password is Wrong!</li>';
					}
				} else {
						...
						<div class="col-lg-5">
							...
							<div class="card shadow-lg border-0 rounded-lg mt-5">
								...
								<div class="card-body">
									<?php
									if ($message != '') {
										echo '<div class="alert alert-danger alert-dismissible fade show"><ul class="list-unstyled">'.$message.'</ul><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
									}
									?>
									<form method="post">
										<div class="mb-3">
